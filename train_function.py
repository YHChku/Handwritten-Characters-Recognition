...
def plot_train_curve(train_info, model_name, learning_rate, batch_size, save_path):
    ...
    plt.savefig(os.path.join(save_path, fig_filename), bbox_inches='tight')
    plt.clf()
...
def write_train_info(train_info, model_name, learning_rate, batch_size, save_path):
    if learning_rate == 0.1:
        lr = '01'
    elif learning_rate == 0.01:
        lr = '001'
    ...
    df.to_csv(os.path.join(save_path, filename), header=False, index=False)
...
def train_and_test(opt, model, criterion, optimizer, train_data_loader, test_data_loader, dataset_sizes,device):
    ...
    for epoch in range(num_epochs):
        ...
        # complete one epoch
        epoch_train_loss = train_loss / dataset_sizes['train_data']
        epoch_train_acc = train_acc / dataset_sizes['train_data']

        epoch_test_loss = test_loss / dataset_sizes['test_data']
        epoch_test_acc = test_acc / dataset_sizes['test_data']

        print('Epoch {}/{}, Train loss: {:.4f}, Train acc: {:.4f}'.format(epoch + 1, num_epochs, epoch_train_loss,
                                                                          epoch_train_acc))
        print('Epoch {}/{}, Test loss: {:.4f}, Test acc: {:.4f}'.format(epoch + 1, num_epochs, epoch_test_loss,
                                                                        epoch_test_acc))
    ...
    write_train_info(train_info, opt.model_name, opt.lr, opt.batch_size, opt.save_path)
    plot_train_curve(train_info, opt.model_name, opt.lr, opt.batch_size, opt.save_path)
